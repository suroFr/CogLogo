...
public class LeaveAllGroups implements Command{
	...
	public void perform(Argument[] args, Context context) throws ExtensionException {
		...
		if(myCogScheme != null)
		{
			...
			{
				try {
					myCogScheme.leaveGroup(((double)t.getVariable(myCogScheme.getGroupVariableOffset(breedName,args[0].getString()))), t);
					t.setVariable(myCogScheme.getCulturonOffset(breedName) + (i*3), -1.0);
					t.setVariable(myCogScheme.getCulturonOffset(breedName) + ((i*3)+1), "_NULL_");
					t.setVariable(myCogScheme.getCulturonOffset(breedName) + ((i*3)+2), 0.0);
