...
public class LeaveGroup implements Command{
	...
	public void perform(Argument[] args, Context context) throws ExtensionException {
		...
		if(myCogScheme != null)
		{
			double groupId = ((double)t.getVariable(myCogScheme.getGroupVariableOffset(breedName,args[0].getString())));
			if(myCogScheme.checkGroupTypeExists(args[0].getString())&& groupId != -1.0)
			{
				try {
					t.setVariable(myCogScheme.getGroupVariableOffset(breedName,args[0].getString()), -1.0);
					t.setVariable(myCogScheme.getGroupVariableOffset(breedName,args[0].getString())+1, "_NULL_");
					t.setVariable(myCogScheme.getGroupVariableOffset(breedName,args[0].getString())+2, 0.0);
					myCogScheme.leaveGroup(groupId, t);
