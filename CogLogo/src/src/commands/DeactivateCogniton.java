...
public class DeactivateCogniton implements Command{
		...
		if(myCogScheme != null)
		{
			try {
				if(t.getVariable(CognitonExtension.getBreedVariableOffset(breedName)+myCogScheme.cognitonNames.indexOf(args[0].getString())).getClass() != String.class)
					t.setVariable(CognitonExtension.getBreedVariableOffset(breedName)+myCogScheme.cognitonNames.indexOf(args[0].getString()), new String("NULL"));				
